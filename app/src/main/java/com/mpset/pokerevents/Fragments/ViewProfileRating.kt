...
class ViewProfileRating : Fragment() {
    var locationId: Int = 0
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
                              savedInstanceState: Bundle?): View? {
        // Inflate the layout for this fragment
        val view = inflater.inflate(R.layout.fragment_view_profile_rating, container, false)
        val id: Int = activity!!.intent.getIntExtra("friend_id", 0)
        getViewProfileInformationRating(SplashScreen.userAccessToken, id)

        view.player_reviews_view_profile.setOnClickListener {
            val intent = Intent(context, ReviewDetailsPage::class.java)
            intent.putExtra("review_id", id)
            intent.putExtra("type", "player")
            startActivity(intent)
        }
        view.host_reviews_view_profile.setOnClickListener {
            val intent = Intent(context, ReviewDetailsPage::class.java)
            intent.putExtra("review_id", id)
            intent.putExtra("type", "host")
            startActivity(intent)
        }
        view.location_reviews_view_profile.setOnClickListener {
            val intent = Intent(context, ReviewDetailsPage::class.java)
    ...
    fun getViewProfileInformationRating(tokenAccess: String, id: Int) {
        ...
        val stringRequest = object : StringRequest(Request.Method.GET, URL, Response.Listener { s ->
            ...
            try {
                ...
                view!!.player_rating_view_profile.rating = avgPlayerRating.toFloat()
                view!!.host_rating_view_profile.rating = avgHostRating.toFloat()
                view!!.location_rating_view_profile.rating = avgLocationRating.toFloat()
